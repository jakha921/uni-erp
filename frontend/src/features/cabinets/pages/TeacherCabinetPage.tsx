import { PageContent, PageHeader } from '@/components/layout';
import { Card } from '@/components/data-display';
import { Button } from '@/components/ui';
import {
  CalendarDays,
  GraduationCap,
  ClipboardCheck,
  ListTodo,
  Calendar,
} from 'lucide-react';

// --- Types ---

interface ClassItem {
  time: string;
  subject: string;
  group: string;
  room: string;
  students: number;
}

interface TaskItem {
  text: string;
  due: string;
  priority: 'high' | 'med' | 'low';
}

// --- Mock Data ---

const TEACHER = {
  name: 'prof. Tursunov R.M.',
  initials: 'TR',
  department: 'Dasturiy injiniring kafedrasi',
  position: 'Professor',
  experience: 23,
  todayClasses: 3,
  totalStudents: 184,
  tasksCount: 14,
};

const TODAY_CLASSES: ClassItem[] = [
  { time: '08:30', subject: 'Algoritmlar', group: '301-A', room: 'A-204', students: 28 },
  { time: '10:10', subject: 'Algoritmlar', group: '302-B', room: 'A-204', students: 26 },
  { time: '13:40', subject: "Ma'lumotlar bazasi", group: '203-A', room: 'A-208', students: 30 },
];

const TASKS: TaskItem[] = [
  { text: 'Algoritmlar — 301-A baholar (oraliq nazorat)', due: 'Bugun', priority: 'high' },
  { text: 'Diplom rahbarligi — Aliyeva N.A. tahriri', due: 'Ertaga', priority: 'med' },
  { text: 'Ilmiy maqola tahriri (CACM)', due: '3 kun', priority: 'low' },
  { text: 'Kafedra yig\'ilishi — chorshanba 14:00', due: '5 kun', priority: 'low' },
];

const QUICK_ACTIONS = [
  { icon: CalendarDays, label: 'Dars jadvali', color: '#3B82F6' },
  { icon: GraduationCap, label: 'Baholar', color: '#2DB976' },
  { icon: ClipboardCheck, label: 'Davomat', color: '#F59E0B' },
  { icon: ListTodo, label: 'Vazifalar', color: '#8B5CF6' },
] as const;

const PRIORITY_COLORS: Record<TaskItem['priority'], string> = {
  high: '#EF4444',
  med: '#F59E0B',
  low: '#94A3B8',
};

// --- Component ---

export function TeacherCabinetPage() {
  return (
    <PageContent>
      <PageHeader
        title="O'qituvchi kabineti"
        breadcrumbs={[{ label: 'Kabinetlar' }, { label: 'O\'qituvchi kabineti' }]}
      />

      {/* Hero Banner */}
      <div className="rounded-2xl bg-gradient-to-br from-blue-700 to-blue-900 p-6 text-white flex items-center gap-5 mb-5">
        <div className="flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-full bg-white/20 text-[26px] font-bold">
          {TEACHER.initials}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-medium uppercase tracking-wider opacity-85">O&apos;qituvchi kabineti</p>
          <h2 className="mt-1 text-2xl font-bold">{TEACHER.name}</h2>
          <p className="mt-1 text-[13px] opacity-90">
            {TEACHER.department} &middot; {TEACHER.position} &middot; {TEACHER.experience} yil tajriba
          </p>
        </div>
        <div className="hidden md:flex items-center gap-8">
          <StatBlock value={String(TEACHER.todayClasses)} label="Bugungi darslar" />
          <div className="h-10 w-px bg-white/30" />
          <StatBlock value={String(TEACHER.totalStudents)} label="Talabalar" />
          <div className="h-10 w-px bg-white/30" />
          <StatBlock value={String(TEACHER.tasksCount)} label="Vazifalar" />
        </div>
      </div>

      {/* Quick Actions */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {QUICK_ACTIONS.map((action) => (
          <button
            key={action.label}
            className="flex items-center gap-3 rounded-2xl bg-white p-4 shadow-[0_1px_3px_rgba(0,0,0,0.08)] text-left hover:shadow-md transition-shadow"
          >
            <div
              className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl"
              style={{ backgroundColor: `${action.color}15`, color: action.color }}
            >
              <action.icon className="h-5 w-5" />
            </div>
            <span className="text-[13px] font-semibold text-slate-900">{action.label}</span>
          </button>
        ))}
      </div>

      {/* Today's Classes + Tasks */}
      <div className="grid grid-cols-1 lg:grid-cols-[1.4fr_1fr] gap-4">
        {/* Today's Classes */}
        <Card>
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-semibold text-slate-900">Bugungi dars jadvali</h3>
              <p className="text-xs text-slate-500 mt-0.5">Chorshanba, 24 aprel</p>
            </div>
            <Button variant="secondary" size="sm">
              <ClipboardCheck className="h-3.5 w-3.5 mr-1" />
              Davomat olish
            </Button>
          </div>
          <div className="flex flex-col gap-2">
            {TODAY_CLASSES.map((cls) => (
              <div
                key={`${cls.time}-${cls.group}`}
                className="flex items-center gap-3.5 rounded-xl bg-[#F8FAFB] p-3"
              >
                <div className="w-14 text-center">
                  <span className="text-base font-bold text-slate-900 tabular-nums">{cls.time}</span>
                </div>
                <div className="h-9 w-px bg-slate-200" />
                <div className="flex-1 min-w-0">
                  <p className="text-[13.5px] font-semibold text-slate-900">{cls.subject}</p>
                  <p className="text-[11.5px] text-slate-500 mt-0.5">
                    {cls.group} &middot; {cls.room} &middot; {cls.students} talaba
                  </p>
                </div>
                <Button variant="secondary" size="sm">Boshlash</Button>
              </div>
            ))}
          </div>
        </Card>

        {/* Tasks */}
        <Card>
          <h3 className="text-sm font-semibold text-slate-900 mb-4">Mening vazifalarim</h3>
          <div className="flex flex-col gap-2">
            {TASKS.map((task) => (
              <div key={task.text} className="flex gap-2.5 rounded-lg bg-[#F8FAFB] p-2.5">
                <div
                  className="w-1 shrink-0 self-stretch rounded-full"
                  style={{ backgroundColor: PRIORITY_COLORS[task.priority] }}
                />
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] text-slate-900 leading-relaxed">{task.text}</p>
                  <p className="text-[11px] text-slate-500 mt-1 flex items-center gap-1">
                    <Calendar className="h-2.5 w-2.5" />
                    {task.due}
                  </p>
                </div>
              </div>
            ))}
          </div>
        </Card>
      </div>
    </PageContent>
  );
}

function StatBlock({ value, label }: { value: string; label: string }) {
  return (
    <div className="text-center">
      <p className="text-[32px] font-bold tabular-nums">{value}</p>
      <p className="text-[11px] opacity-80 mt-0.5">{label}</p>
    </div>
  );
}
