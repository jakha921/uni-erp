...
const TYPE_KEYS: Record<Conference['type'], string> = {
  international: 'science.typeInternational',
  national: 'science.typeNational',
  university: 'science.typeUniversity',
};
...
export function ConferencesPage() {
  ...
  return (
    <PageContent>
      <PageHeader
        title={t('science.conferencesTitle')}
        subtitle={t('science.conferencesSubtitle')}
        breadcrumbs={[{ label: t('nav.science') }, { label: t('nav.conferences') }]}
        actions={
          <Button variant="primary" size="sm" onClick={() => setFormOpen(true)}>
            <Plus className="h-4 w-4 mr-1.5" /> {t('science.newConference')}
          </Button>
        }
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
        <StatCard label={t('science.totalConferences')} value={(conferencesData?.total ?? allConferences.length).toString()} icon={<Presentation className="h-5 w-5" />} />
        <StatCard label={t('science.planned')} value={upcomingList.length.toString()} icon={<Calendar className="h-5 w-5" />} trend={{ value: 2 }} />
        <StatCard label={t('science.totalParticipants')} value={totalParticipants.toLocaleString()} icon={<Users className="h-5 w-5" />} />
        <StatCard label={t('science.international')} value={international.toString()} icon={<Globe className="h-5 w-5" />} trend={{ value: 1 }} />
      </div>
      ...
      {isLoading ? (
        ...
        <Card title="" className="overflow-hidden">
          <div className="border-b border-border px-4 flex items-center gap-1">
            {([['upcoming', t('science.tabUpcoming')], ['past', t('science.tabPast')]] as [Tab, string][]).map(([tabId, label]) => (
              <button
                key={tabId}
                onClick={() => setTab(tabId)}
                className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${
                  tab === tabId ? 'border-primary-500 text-primary-600' : 'border-transparent text-slate-600 hover:text-slate-900'
                }`}
              >
                {label}
                <span className="ml-2 text-xs text-slate-400">({(tabId === 'upcoming' ? upcomingList : pastList).length})</span>
              </button>
            ))}
          </div>
            ...
            {list.map((conf) => (
              <div key={conf.id} className="rounded-xl border border-border bg-white p-4 hover:shadow-sm transition-shadow">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${TYPE_COLOR[conf.type] ?? ''}`}>
                      {t(TYPE_KEYS[conf.type]) ?? conf.type}
                    </span>
                ...
                <div className="flex items-center gap-4 text-xs text-slate-500">
                  ...
                  <span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{t('science.participantCount', { count: conf.participantCount })}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <ConfirmDialog
        ...
        title={t('science.deleteConferenceTitle')}
        message={t('science.deleteConferenceConfirm', { name: deleteConference?.name })}
